...
An important detail to mention is that the DJIMotor class has a `bool useAbsEncoder` with default value `true`. This is used when working with position PID. When this variable is set to `false`, the PID will use `MULTITURNANGLE` to calculate the power output, this means that if our target is 8192 ticks and the motor's current position is 0 ticks, it will do a full revolution. If our target is 2 * 8192 it will do two revolutions and so on. On the other hand, if this is set to `true`, the motor only cares about the current `ANGLE`, this means if our target is 8192 and the motor's current position is 0 ticks, the motor will not move, since 8192 wraps around to 0. If our target is 2 * 8192 it won't move either, since again, this wraps around to 0. This version of the position PID implements a smart `s_calculateDeltaPhase`, which returns the smallest `Δthicks`, either clockwise or counterclockwise.
<br>
